// This is the dictionary file for the link below:
// https://drstw.github.io/index.html
// en: 
// jp: completed on 2020/03/13 by Adam36

const messages = {
    tw: {
        message: {
            navbar: {
                title: "DANCERUSH 台灣交流會",
                home: "首頁",
                TOC: "線上交流賽 (TOC)",
                TOCmain: "競賽首頁",
                schedule: "競賽時程",
                songs: "競賽指定樂曲",
                contestants: "選手名單",
                rules: "詳細規則與說明",
                faq: "常見問題",
                links: "重要連結",
                HOF: "線上賽名人堂",
                language: "🇹🇼 繁體中文",
                tw: "🇹🇼 繁體中文",
                en: "🇺🇸 English",
                jp: "🇯🇵 日本語"
            },
            main: {
                welcome: "歡迎來到 DANCERUSH 台灣交流會！",
                welcomemsg01: "這裡是一個讓各地的 DANCERUSH STARDOM 玩家可以互相交流的地方。",
                welcomemsg02: "除了機台遊戲最新的資訊，也會不定期舉辦線上與線下活動。最重要的是定期的線上交流賽 (TOC)，能夠與不同國家的玩家切磋，不管是誰都可以參與！"
            },
            TOC: {
                title: {
                    t1: "DANCERUSH STARDOM 台灣線上交流賽 2021",
                    t2: "DANCERUSH STARDOM Taiwan Online Contest 2021 (TOC2021)"
                },
                alert: {
                    heading: "請注意",
                    content: "由於新型冠狀病毒 (COVID-19) 之疫情有日漸升高的趨勢，請不要忽略任何導致疫情擴散的可能性。在不得已之情況下，主辦單位可能會依據疫情狀況，提前截止甚至結束賽事進行。請各位玩家務必於遊玩前、中、後做好防疫措施，戴上口罩與勤洗手，確保大家的健康與安全。你的小小一份努力，能夠讓所有的舞友、甚至全台灣的民眾在未來能夠維持正常的娛樂與身心活動。"
                },
            },
            social: {
                youtube: "DANCERUSH 台灣交流會"
            },
            footer: {
                contact: "本網頁由 Nerova (@NerovaRiuzGX) 架設。如果操作過程中遇到任何問題，請聯絡 DANCERUSH 台灣交流會官方帳號。"
            }
        }
    },

    en: {
        message: {
            navbar: {
                title: "DANCERUSH Taiwan",
                home: "Home",
                TOC: "Taiwan Online Contest (TOC)",
                TOCmain: "Page Top",
                schedule: "Contest Schedule",
                songs: "Entry Songs",
                contestants: "List of Contestants",
                rules: "Rules & Details",
                faq: "FAQs",
                links: "Important Links",
                HOF: "TOC Hall of Fame",
                language: "🇺🇸 English",
                tw: "🇹🇼 繁體中文",
                en: "🇺🇸 English",
                jp: "🇯🇵 日本語"
            },
            main: {
                welcome: "Welcome to DANCERUSH Taiwan!",
                welcomemsg01: "It's a place for DANCERUSH STARDOM players to communicate.",
                welcomemsg02: "Besides the latest information of DANCERUSH, online and offline events would be held. Most important of all, we'll hold the Taiwan Online Contest(TOC). You can competite with players from different countries, and everyone is welcomed to participate!"
            },
            TOC: {
                title: {
                    t1: "DANCERUSH STARDOM Taiwan Online Contest 2021",
                    t2: "(TOC2021)"
                },
                alert: {
                    heading: "Attention:",
                    content: "Due to the raising of COVID-19 pandemic, the contest would be cancelled in the case of emergency. Please put on your mask and wash your hands to be a hero in this pandemic."
                },
            },
            social: {
                youtube: "DANCERUSH Taiwan"
            },
            footer: {
                contact: "This site is built by Nerova (@NerovaRiuzGX). If you have any problem during visiting, please feel free to contact us in the social media above."
            }
        }
    },

    jp: {
        message: {
            navbar: {
                title: "ダンスラ台湾",
                home: "トップ",
                TOC: "台湾オンラインコンテスト (TOC)",
                TOCmain: "トップページ",
                schedule: "スケジュール",
                songs: "課題曲",
                contestants: "参加者",
                rules: "ルール",
                faq: "よくある質問",
                links: "リンク",
                HOF: "栄誉殿堂",
                language: "🇯🇵 日本語",
                tw: "🇹🇼 繁體中文",
                en: "🇺🇸 English",
                jp: "🇯🇵 日本語"
            },
            main: {
                welcome: "ダンスラ台湾へようこそ！",
                welcomemsg01: "ここにはダンスラのプレイヤー達が交流することができます。",
                welcomemsg02: "最新のダンスラ情報以外、オンラインと店舗大会も開催します。特にオンラインコンテスト (TOC)は海外のプレイヤーと競うことができます。誰でも参加します！"
            },
            TOC: {
                title: {
                    t1: "DANCERUSH STARDOM 台湾オンラインコンテスト 2021",
                    t2: "DANCERUSH STARDOM Taiwan Online Contest 2021 (TOC2021)"
                },
                alert: {
                    heading: "ご注意ください",
                    content: "今はまたコロナの拡散可能性があります。念のために、パンデミックによって、大会は停止となります。ゲームセンターがいる時に、必ずマスクを着用して、手を洗うしてください。小さな一歩で、自身と他人を守ることができます。"
                },
            },
            social: {
                youtube: "ダンスラ台湾チャンネル"
            },
            footer: {
                contact: "サイト管理者：Nerova (@NerovaRiuzGX)。質問があるの場合は、上記のSNSでお気軽にお問い合わせください。"
            }
        }
    }
}
